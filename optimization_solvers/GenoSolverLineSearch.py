...
if os.path.exists(os.path.join(HERE_TEST_PATH, '../genosolver_line_search')):
    sys.path.insert(0, os.path.join(HERE_TEST_PATH, '../genosolver_line_search/'))
    from genosolver.genosolver import minimize
    sys.path.pop(0)
else:
    raise ImportError('No genosolver_line_search folder')


class GenoSolverLineSearch(BaseSolver):
    """GenoSolver wrapper.
    """
