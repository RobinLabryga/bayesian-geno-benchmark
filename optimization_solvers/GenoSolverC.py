...
if os.path.exists(os.path.join(HERE_TEST_PATH, '../genosolver_c')):
    sys.path.insert(0, os.path.join(HERE_TEST_PATH, '../genosolver_c/'))

    from genosolver_c.genosolver import minimize
    sys.path.pop(0)
else:
    raise ImportError('No genosolver_c folder')


class GenoSolverC(BaseSolver):
    """Old Genosolver wrapper.
    """
    ...
    def solve(self):
        ...
        res = minimize(self.fg_with_timing, x, bounds=bounds, options=self.options, constraints=constraints)
