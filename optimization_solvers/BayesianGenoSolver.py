...
if os.path.exists(os.path.join(HERE_TEST_PATH, "../bayesian-geno/")):
    ...
    from bayesian_line_search.GPgenosolver import minimize
...
class BayesianGenoSolver(BaseSolver):
    """BayesianGenosolver wrapper."""
    ...
    def solve(self):
        ...
        if (
            self.optimization_problem.cl is None
            and self.optimization_problem.cu is None
        ):
            constraints = None
        else:
            constraints = dict()
            ...
            constraints["jacprod"] = lambda x, v: self.optimization_problem.jprod(
                v, transpose=True, x=x
            )
