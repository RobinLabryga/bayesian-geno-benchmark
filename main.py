...
from utils import save_results, setup_from_config_file, create_failed_result, create_additional_info
...
if __name__ == '__main__':
    ...
    parser.add_argument('--no-save', action='store_true', help='Do not save to results/ directory.')
    parser.add_argument('--single_fig_per_f_over_time', action='store_true',
                        help='Save one f_over_time plot per file.')
    parser.add_argument('--barplots_cnt', type=int, default=-1,
    ...
    parser.add_argument('--result_dir', type=str,
                        help='Name of the directory inside results/')
    parser.add_argument('--print_results', action='store_true', help='TBA')
    args = parser.parse_args()

    config = args.config
    problem_dict, solver_dict, problems, solvers = setup_from_config_file(config, available_problems, available_solvers)
    ...
    for problem_name in tqdm(problems):
        ...
        for solver_name in solver_dict.keys():
            problem = pycutest.import_problem(problem_name, sifParams=problem_dict[problem_name])
            ...
            try:
                ...
                if args.profile:
                    ...
                    s.solve()
                tqdm.write(f'Finished {problem.name} with {solver.__name__} in {s.getResult().time} seconds')
                res = s.getResult()
            except TimeoutError:
                tqdm.write(f'Timed out on {problem_name} with {solver_name}')
                res = create_failed_result(problem, status=101, message='Timed out', xs=s.xs, fs=s.fs, ts=s.ts, pgs=s.pgs)
            except KeyboardInterrupt:
                tqdm.write(f'Interrupted {problem_name} with {solver_name}')
                res = create_failed_result(problem, status=102, message='Keyboard interrupt', xs=s.xs, fs=s.fs, ts=s.ts, pgs=s.pgs)
            except BaseException as error:
                tqdm.write(f'Broke on {problem_name} with {solver_name}\nError message: {error}')
                res = create_failed_result(problem, status=103, message=str(error), xs=s.xs, fs=s.fs, ts=s.ts, pgs=s.pgs)
    ...
    if not args.no_save:
        ...
        save_results(results, metrics, solver_dict, problem_dict, one_fig_pdf=(not args.single_fig_per_f_over_time), barplots_cnt=args.barplots_cnt, result_dir=args.result_dir)
