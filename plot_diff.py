...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="For plotting .pkl results of runs.",
    )
    parser.add_argument(
        "--paths",
        "--pth",
        ...
        nargs="+",
        required=True,
        help="Paths to all results to be plotted.",
    )
    parser.add_argument(
        "--out",
        type=str,
        default="./diff_plots/",
        help="PDF directory where to save plotted results.",
    )
    ...
    for indx, results in enumerate(all_results):
        for task in results:
            compact_results[task] |= {
                (solver + str(indx)): res for (solver, res) in results[task].items()
            }

    for task in compact_results:
        for solver in compact_results[task]:
            compact_results[task][solver]["ts"] = list(
                range(len(compact_results[task][solver]["ts"]))
            )

    os.makedirs(args.out, exist_ok=True)
    plot_metrics_barplot(compact_results, ["nfev", "nit"], args.out)
    plot_f_over_time(compact_results, args.out)
    plot_f_over_feval(compact_results, args.out)
